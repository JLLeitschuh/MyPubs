describe('Tests for pw.editContributor', function() {
    ...
    describe('Tests for editContributorCtrl', function() {
		...
		beforeEach(function() {
			...
			mockLookupFetcher.promise.andCallFake(function(lookup, params) {
				var result = [];
			...
			spyOn(mockContributorPersister, 'persistContributor').andCallThrough();
		});
		...
		beforeEach(inject(function($injector) {
			...
			createController = function(thisContributor) {
				return $controller('editContributorCtrl', {
					...
					'ContributorPersister' : mockContributorPersister
				});
			};
		}));
		...
		it('Expects that controller initialized with a non USGS contributor assigns the outside affiliations to affiliations', function() {
			...
			expect(mockLookupFetcher.promise).toHaveBeenCalledWith('outsideaffiliates', {active : 'y'});
			expect(mockLookupFetcher.promise).toHaveBeenCalledWith('outsideaffiliates', {active : 'n'});
			expect($scope.activeAffiliations).toEqual(OUTSIDE_AFFILIATIONS);
			expect($scope.notActiveAffiliations).toEqual(OUTSIDE_AFFILIATIONS);
		});
		...
		it('Expects that controller initialized with a USGS contributor assigns cost center to affiliations', function() {
			...
			expect(mockLookupFetcher.promise).toHaveBeenCalledWith('costcenters', {active : 'y'});
			expect(mockLookupFetcher.promise).toHaveBeenCalledWith('costcenters', {active : 'n'});
			expect($scope.activeAffiliations).toEqual(COST_CENTERS);
			expect($scope.notActiveAffiliations).toEqual(COST_CENTERS);
		});
		...
		it('Expects that a save for a new contributor changes the path to use the returned data\'s contributorId and to clear validationErrors', function() {
			...
			$scope.saveChanges();
			expect(mockContributorPersister.persistContributor).toHaveBeenCalled();
			$scope.$digest();
